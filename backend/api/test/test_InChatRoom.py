...
def test_response_chat_room_not_exist(usernames, userpasswords, room_name, password):
    create_room_url = reverse('create_chat_room')
    users = [User.objects.create_user(username=username, password=userpassword) for username, userpassword in zip(usernames, userpasswords)]
    room_data = {
        'room_name': room_name, 
    ...
    check_user_in_room_url = reverse('user_in_chat_room')
    for username in usernames:
        response: Response = api_client.post(check_user_in_room_url, data={'username': username, 'room_name': 'none_existing_room'})
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert response.data['detail'] == 'This chat room does not exist.'